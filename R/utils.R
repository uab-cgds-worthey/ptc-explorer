  ...
  tryCatch(
    {
      ...
      if (httr::status_code(response) == 200) {
        ...
        if (is.null(parsed_data) || length(parsed_data) == 0) {
          return(HTML(paste0(
            "<strong>No Information Found:</strong> No data available for gene '",
            gene_clean, "'."
          )))
        }
        ...
        return(HTML(paste0(
          "<strong>API Error:</strong> Status code ",
    ...
    error = function(e) {
      return(HTML(paste0(
        "<strong>Error:</strong> Unable to fetch gene information for '",
